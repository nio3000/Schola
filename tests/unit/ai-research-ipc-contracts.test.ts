...
describe('AI Research IPC — 白名单通道定义', () => {
  const ALLOWED_CHANNELS = [
    { name: 'AI_RESEARCH_GET_PROVIDER_READINESS_CHANNEL', value: AI_RESEARCH_GET_PROVIDER_READINESS_CHANNEL, expected: 'ai-research:get-provider-readiness' },
    { name: 'AI_RESEARCH_BUILD_CONTEXT_PACK_CHANNEL', value: AI_RESEARCH_BUILD_CONTEXT_PACK_CHANNEL, expected: 'ai-research:build-context-pack' },
    { name: 'AI_RESEARCH_PREVIEW_CONTEXT_PACK_CHANNEL', value: AI_RESEARCH_PREVIEW_CONTEXT_PACK_CHANNEL, expected: 'ai-research:preview-context-pack' },
    { name: 'AI_RESEARCH_CREATE_TASK_DRAFT_CHANNEL', value: AI_RESEARCH_CREATE_TASK_DRAFT_CHANNEL, expected: 'ai-research:create-task-draft' },
    { name: 'AI_RESEARCH_RUN_CONFIRMED_TASK_CHANNEL', value: AI_RESEARCH_RUN_CONFIRMED_TASK_CHANNEL, expected: 'ai-research:run-confirmed-task' },
    { name: 'AI_RESEARCH_CANCEL_TASK_CHANNEL', value: AI_RESEARCH_CANCEL_TASK_CHANNEL, expected: 'ai-research:cancel-task' },
    { name: 'AI_RESEARCH_GET_TASK_STATUS_CHANNEL', value: AI_RESEARCH_GET_TASK_STATUS_CHANNEL, expected: 'ai-research:get-task-status' },
    { name: 'AI_RESEARCH_GET_TASK_RESULT_CHANNEL', value: AI_RESEARCH_GET_TASK_RESULT_CHANNEL, expected: 'ai-research:get-task-result' },
    { name: 'AI_RESEARCH_CLEAR_TASK_RESULT_CHANNEL', value: AI_RESEARCH_CLEAR_TASK_RESULT_CHANNEL, expected: 'ai-research:clear-task-result' },
    { name: 'AI_RESEARCH_DISCARD_ARTIFACT_CHANNEL', value: AI_RESEARCH_DISCARD_ARTIFACT_CHANNEL, expected: 'ai-research:discard-artifact' },
  ];

  it('共定义了 10 个 AI Research IPC 通道常量', () => {
    assert.equal(ALLOWED_CHANNELS.length, 10, '应恰好定义 10 个通道常量');
  });
...
describe('AI Research IPC — 禁止通道验证', () => {
  ...
  const IPC_SOURCES = [
    ...
    MAIN_FILE,
    PRELOAD_FILE,
    'electron/ipc/ai-research.ipc.ts',
  ];
    ...
    it(`禁止通道 "${forbidden}" 未在任何 IPC 源文件中注册`, () => {
      ...
      for (const src of IPC_SOURCES) {
        ...
        assert.ok(
          ...
          `禁止通道 "${forbidden}" 在 ${src} 中被发现 — 不得注册`,
        );
      }
    });
  }
...
describe('AI Research IPC — 无通用 invoke 通道', () => {
  ...
  it('ai-research.types.ts 的 ScholaAIResearchApi 接口不含通用 invoke 方法', () => {
    ...
    assert.ok(
      !/invoke\s*\(/.test(apiBody),
      'ScholaAIResearchApi 不得包含通用 invoke 方法',
    );
  });
  ...
  it('ai-research.types.ts 不含 "ai-research:" 以外的通道前缀', () => {
    const content = readSource(CONTRACTS_FILE);
    assert.ok(content);
    const channelAssignments = content!.matchAll(
